...
export default function OrdersPage() {
  ...
  const updateStatus = async (orderId: string, status: string) => {
    try {
      ...
      showToast(`Commande ${status === "confirmed" ? "confirmée" : status === "delivered" ? "livrée" : "annulée"}`);
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">{orders.length} commandes | {pendingCount} en attente</p>
          </div>
          <Card className="!p-0">
                ...
                {selectedOrder.status === "confirmed" && (
                  <>
                    ...
                    <Button size="sm" variant="secondary" onClick={() => printOrder(selectedOrder)}>
                      <Printer className="h-3 w-3 mr-1" /> Bon de livraison
                    </Button>
                  </>
                )}
              </div>
